...
typedef NS_ENUM(NSInteger, JAALEE_BUTTON_TYPE) {
    JAALEE_BUTTON_TYPE_SINGLE = 0,
    JAALEE_BUTTON_TYPE_LONG_PRESS,
};


typedef NS_ENUM(NSInteger, ADV_STATE_WHEN_DOUBLE_CLICK_THE_BUTTON) {
    ADV_IBEACON_CHANNEL = 0, //Advertise iBeacon channel's data when double click the button of JAALEE-OFA
    ADV_CUSTOM_CHANNEL,    //Advertise Custom channel's data when double click the button of JAALEE-OFA
    ADV_DISABLE,//Disabled
};


typedef NS_ENUM(NSInteger, JAALEE_BEACON_HARDWARE_VERSION) {
    NORMAL_BEACON_SERIES_N = 0, //iB001-N, iB004-N, USB Beacon
    NORMAL_BEACON_SERIES_T,    //TI Version
    NORMAL_BEACON_iB003N,
    NORMAL_BEACON_JAALEE_OFA,
};
