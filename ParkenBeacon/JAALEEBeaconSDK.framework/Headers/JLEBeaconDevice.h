...
#pragma mark *** Delegate Method for Connection ***
...
#pragma mark *** Delegate Method for iB003-N with Accelerometer ***
/**
...
#pragma mark *** Delegate Method for iB003-N & JAALEE-OFA ***
 ...
 * @return void
 */
- (void)beaconDidMotionDetected:(JLEBeaconDevice*)beacon;


#pragma mark *** Delegate Method for JAALEE-OFA ***

/**
 * Delegate method that JAALEE's button touched.
 *
 * @param beacon reference to beacon object
 *
 * @return void
 ...
 *
 *  Discussion:
 *    Local name of the beacon device.
 */
 ...
 *    Reference of the beacon device peripheral object.
 */
@property (nonatomic, readonly)   JAALEE_BEACON_HARDWARE_VERSION  hardwareVersion;
 ...
 *  Button detect state
 */
@property (nonatomic, readonly)   BOOL                                       isButtonDetecting;
 ...
 *  isSupportEddystone
 *
 *  Is support eddystone
 */
@property (nonatomic, readonly)   BOOL                                     isSupportEddystone;
 ...
 *  Is support Button
 */
@property (nonatomic, readonly)   BOOL                                     isSupportButton;
 ...
 *
 * @param interval beacon (100 - 10000 ms)
 * @param completion block handling operation completion
 *
 * @return void
 */
...
/**
 *  Configure custom channel's data as Eddystone-URL.
 *
 ...
 * @return void
 */
- (void) configureCustomChannelAsEddyStoneURL:(NSString*)url txPower:(int)txPower WithCompletion:(JLEBoolCompletionBlock)completion;


/**
 *  Configure custom channel's data as Eddystone-UID.
 *
 * @param namespaceID namespace id with eddystone
  * @param InstanceID instance id with eddystone
 * @param measuredPowerValue this value just for measured the distance, it would not change the transmition distance of the beacon
 * @param completion block handling operation completion
 *
 * @return void
 */
- (void) configureCustomChannelAsEddyStoneUID:(NSString*)namespaceID InstanceID:(NSString*)InstanceID measuredPowerValue:(int)measuredPowerValue WithCompletion:(JLEBoolCompletionBlock)completion;

/**
 *  Configure custom channel's data as iBeacon data.
 *
 * @param UUID  ibeacon's proximity UUID
 * @param major ibeacon's major value
 * @param minor ibeacon's minor value
 * @param measuredPowerValue iBeacon measured power value
 * @param completion block handling operation completion
 *
 * @return void
 */
- (void) configureCustomChannelAsiBeacon:(NSString*)UUID major:(unsigned short)major minor:(unsigned short)minor measuredPowerValue:(unsigned short)measuredPowerValue WithCompletion:(JLEBoolCompletionBlock)completion;


/**
 *  Enable Eddystone Only, if you want use both of iBeacon & Eddystone please use the method: writeBeaconState:connectable:withCompletion:
 *
 * @param connectable Connectable when beacon is broadcast.
 * @param completion block handling operation completion
 *
 * @return void
 */
- (void) configureCustomChannelAlwaysBroadcast:(BOOL)connectable WithCompletion:(JLEBoolCompletionBlock)completion;


#pragma mark *** Local method for iB003-N only ***
...
#pragma mark *** Local method for JAALEE-OFA only ***

/**
 * Start detect button touch
 *
 * @return Wether the method is call success
 */
- (BOOL) startNotifyJAALEEButton;

/**
 * Stop detect button touch
 *
 * @return Wether the method is call success
 */
- (BOOL) stopNotifyJAALEEButton;

/**
 * Configuring JAALEE's broadcast type when double-click the button
 *
 * state Select the channel you want to broadcast.
 *
 * @return
 */
- (void) configureJAALEEStateWhenDoubleClicktheButton:(ADV_STATE_WHEN_DOUBLE_CLICK_THE_BUTTON)state WithCompletion:(JLEBoolCompletionBlock)completion;

/**
 * Configuring JAALEE's broadcast type when motion entered, this method will reset the beacon state, and JAALEE will broadcast only when detect motion.
 *
 * iBeaconChannleEnable Broadcast ibeacon channel when motion entered.
 * CustomChannelEnable Broadcast custom channel when motion entered.
 * Connectable is connectable when broadcast.
 *
 * @return
 */
- (void) configureJAALEEAdvInMotion:(BOOL)iBeaconChannleEnable CustomChannelEnable:(BOOL)CustomChannelEnable Connectable:(BOOL)Connectable WithCompletion:(JLEBoolCompletionBlock)completion;
@end
